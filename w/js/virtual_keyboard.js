...
function triggerKeyboard(type){
	...
	$('input[type="text"]').after('<div id="vKeyboard">'+getHTMLVirtualKeyboard(type)+'</div>');
	if(type==1){
		$("#vKeyboard").width("100px");
	}else if (type==2){
	...
	$("#"+vKeyboard.saTypeQuesID).mousemove(function(){
		...
		if(vKeyboard.keyProceed){
			...
			if(vKeyboard.selectedKeyboard == 1){
				...
				if((code >47 && code<58) || code==46 || code==45 || code==110 || code==109){
				...
				var code = (event.keyCode ? event.keyCode : event.which);
				if((code >47 && code<58) || (code> 96 && code<123 ) || (code>64 && code<91) || code==45 || code ==43 || code == 46 || code==110 || code==109 || code==107 || code == 32){
					validateAndFill(String.fromCharCode(code));
					//return false;
				}else if(code==8){
					bkspPressed();
